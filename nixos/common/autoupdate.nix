  ...
  options = {
    server_base.autoupdate = {
      ...
      operation = lib.mkOption {
        ...
        default = "switch";
        description = "Whether to use 'switch' (immediate) or 'boot' (on next reboot)";
      };
    };
  };

  config = lib.mkIf config.server_base.autoupdate.enable (
    lib.mkMerge [
      {
      ...
      (lib.mkIf config.server_base.autoupdate.notifyOnFailure {
        ...
        systemd.services.nixos-upgrade.onFailure = [ "nixos-autoupdate-failure-notify.service" ];
        ...
        systemd.services.nixos-autoupdate-failure-notify = {
          ...
          serviceConfig = {
            Type = "oneshot";
            User = "root";
            ExecStart = ''${config.server_base.telegram-notify.script} -s "⚠️ NixOS autoupdate failed on the host `${hostName}`"'';
            TimeoutStartSec = "2s";
            Restart = "on-failure";
            RestartSec = "1s";
            StartLimitBurst = 3;
